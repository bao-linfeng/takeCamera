...
export default {
  ...
  setup(props, context) {
    const { isAdmin, code, siteKey, userKey, deviceID, deviceKey, device, drive, userName } = toRefs(useState());
    ...
    const initSocketIO = () => {
      ...
      socket.on("returnGC", (data) => {
        ...
        let notification = new Notification('消息通知', {'body': data.log});
        notification.onclick = () => {
          router.push('/imageCenter?g='+data.catalogKey+'&v='+data.volumeKey+'&s=');
        }
      });

      socket.on("passGC", (data) => {
        console.log("passGC", data);
        createMsg(data.log, true);
      ...
      socket.on("notReadMessageNumber", (data) => {//未读消息
        console.log(data);
        changePropertyValue('notReadMsgNumber', data.notReadMessageNumber);
      });
...
.prompt-box-module.success p{
	color: #85B83E;
}
.prompt-box-module.warn p{
	color: orange;
}
.prompt-box-module.error p{
	color: #f00;
}
</style>